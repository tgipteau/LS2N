...
def lister_dossiers_simulations():
    ...
    dossiers = [d for d in os.listdir(DOSSIER_SIMULATIONS)
...
def creer_nouvelle_simulation(parametres):
    ...
    label_loading_sim = tk.Label(root,
                                 text=f"Construction de la simulation \"{sim_name}\"...",
                                 font=("Arial", 14))
...
def charger_simulation_existante(sim_name):
    ...
    label_loading_sim = tk.Label(root,
                                 text=f"Chargement de la simulation \"{sim_name}\"...",
                                 font=("Arial", 14))
    label_loading_sim.pack(pady=10)
    root.update_idletasks()
...
def afficher_formulaire_creation():
    ...
    # Bouton Valider
    btn_valider = tk.Button(root, text="Valider",
                            command=lambda: valider_formulaire(champs))
...
def pygame_time(sim_name):
    ...
    pygame_thread = threading.Thread(target=lambda: subprocess.run([sys.executable, "3_fiso2.py", sim_name]))
    pygame_thread.start()
