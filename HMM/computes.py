...
def get_dfs(sim_name, from_folder="Simulations"):
    df = pd.read_csv(os.path.join(from_folder, "data"+sim_name+".csv"),
                     names=["t", "x", "y", "ub", "wb", "um", "wm"], header=0, sep=" ", index_col=False)
    df = df.drop_duplicates()
    
    df_feux = pd.read_csv(os.path.join(from_folder, "fires"+sim_name+".dat"),
...
def plots(Ns, T, val_span, uplus, wplus):
    fig, ax = plt.subplots()
    ...
    plt.show()
    fig.savefig('Figs/distance_curve_eq.png', bbox_inches='tight')
